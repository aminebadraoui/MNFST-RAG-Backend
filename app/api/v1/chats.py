...
async def get_chats(
    ...
    """
    try:
        chats = chat_service.get_tenant_chats(str(current_user.tenant_id))
        
        # Get stats for each chat
        chats_with_stats = []
        for chat in chats:
            stats = chat_service.get_chat_stats(str(chat.id))
            chat_read = ChatRead(
                id=chat.id,
                name=chat.name,
...
async def get_chat(
    ...
    """
    try:
        chat = chat_service.get_chat_by_id(chatId)
        if not chat:
            raise HTTPException(
...
async def delete_chat(
    ...
    try:
        ...
        deleted = chat_service.delete_chat_cascade(chatId)
        if not deleted:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Chat not found"
